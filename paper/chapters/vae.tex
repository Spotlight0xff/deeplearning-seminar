...
The proposed framework includes in addition to an differentiable generator network an inference network in order to perform efficient inference.
...
However it is not appearent how this model would provide sufficiently good results. In order to understand how well the variational autoencoder performs, it is important to understand the variational methods used to build the model.
...
As such variational based methods have become a popular choice to deal with intractable posterior distributions occuring in inference problems.
